...
import { validationForPassword, validationForEmail, validationForWord } from "@/lib/seculity/validation";
...
export default function SignUp() {
    ...
    const handleAction = (formData:FormData) => {
        ///////////
        //◆【formDataのバリデーション】
        const {name,email,password} = signForm;
        //name
        ...
        //name,email,passwordのvalidation結果を反映
        if(name[1] || email[1] || password[1]){
            setSignForm({name,email,password});
            ...
            <div className="flex flex-col items-center justify-center w-full max-w-md">
                {!state.success
                    ?(<> 
                        ...
                        <MailAuth email={signForm.email[0]} typeValue={'SignUp'}/>
                    )
                }
            </div>
        </div>
